...
class MakBotLFreeEnv(LeggedRobot):
    ...
    def _push_robots(self):
        ...
        self.rand_push_force[:, :2] = torch_rand_float(
                -max_vel, max_vel, (self.num_envs, 2),
                device=self.device)  # lin vel x/y
        self.root_states[:, 7:9] = self.rand_push_force[:, :2]

        self.rand_push_torque = torch_rand_float(
                -max_push_angular, max_push_angular, (self.num_envs, 3),
                device=self.device)
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[
        35: 38] = noise_scales.ang_vel * self.obs_scales.ang_vel  # ang vel
        noise_vec[
        38: 41] = noise_scales.quat * self.obs_scales.quat  # euler x,y
        return noise_vec
    ...
    def step(self, actions):
        ...
        delay = torch.rand((self.num_envs, 1),
                           device=self.device) * self.cfg.domain_rand.action_delay
        actions = (1 - delay) * actions + delay * self.actions
        actions += self.cfg.domain_rand.action_noise * torch.randn_like(
            actions) * actions
    ...
    def compute_observations(self):
        ...
        obs_buf = torch.cat((
            self.command_input,
            # 5 = 2D(sin cos) + 3D(vel_x, vel_y, aug_vel_yaw)
            q,  # 12D
            dq,  # 12D
            self.actions,  # 12D
            self.base_ang_vel * self.obs_scales.ang_vel,  # 3
        ...
        self.privileged_obs_buf = torch.cat([self.critic_history[i] for i in
                                             range(self.cfg.env.c_frame_stack)],
                                            dim=1)
    ...
    def _reward_joint_pos(self):
        ...
        r = torch.exp(-2 * torch.norm(diff, dim=1)) - 0.2 * torch.norm(diff,
                                                                       dim=1).clamp(
            0, 0.5)
        return r
    ...
    def _reward_feet_distance(self):
        ...
        return (torch.exp(-torch.abs(d_min) * 100) + torch.exp(
            -torch.abs(d_max) * 100)) / 2
    ...
    def _reward_orientation(self):
        ...
        orientation = torch.exp(
            -torch.norm(self.projected_gravity[:, :2], dim=1) * 20)
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum((torch.norm(
                ...
                dim=-1) - self.cfg.rewards.max_contact_force).clip(0, 400),
                         dim=1)
    ...
    def _reward_default_joint_pos(self):
        ...
        # 左腿关节 (0-4)
        left_yaw = joint_diff[:, 0]  # left_hip_yaw
        left_roll = joint_diff[:, 1]  # left_hip_roll
        left_pitch = joint_diff[:, 2]  # left_hip_pitch
        left_knee = joint_diff[:, 3]  # left_knee
        left_ankle = joint_diff[:, 4]  # left_ankle

        # 右腿关节 (5-9)
        right_yaw = joint_diff[:, 5]  # right_hip_yaw
        right_roll = joint_diff[:, 6]  # right_hip_roll
        right_pitch = joint_diff[:, 7]  # right_hip_pitch
        right_knee = joint_diff[:, 8]  # right_knee
        right_ankle = joint_diff[:, 9]  # right_ankle

        # 计算稳定性偏差(yaw和roll)
        stability_deviation = torch.sqrt(
            left_yaw ** 2 + left_roll ** 2 + right_yaw ** 2 + right_roll ** 2)
        stability_deviation = torch.clamp(stability_deviation - 0.05, 0, 50)

        # 计算姿态偏差(pitch和knee)
        posture_deviation = torch.sqrt(
            left_pitch ** 2 + left_knee ** 2 + right_pitch ** 2 + right_knee ** 2)

        # 计算踝关节偏差
        ankle_deviation = torch.sqrt(left_ankle ** 2 + right_ankle ** 2)

        # 组合奖励
        stability_reward = torch.exp(-stability_deviation * 120)
        posture_reward = torch.exp(-posture_deviation * 80)
        ankle_reward = torch.exp(-ankle_deviation * 60)
    ...
    def _reward_vel_mismatch_exp(self):
        ...
        ang_mismatch = torch.exp(
            -torch.norm(self.base_ang_vel[:, :2], dim=1) * 5.)
    ...
    def _reward_feet_clearance(self):
        ...
        rew_pos = torch.abs(
            self.feet_height - self.cfg.rewards.target_feet_height) < 0.01
